# Awesome Input Device Testing

A curated list of tools for testing **input devices** such as mouse, keyboard, and audio.  
Includes browser-based tools and lightweight utilities.

---

## Mouse Testing

- **GearTest – Mouse Test**  
  Online mouse test for checking click response and detecting double-click issues.  
  https://www.geartest.online/mouse-test/

- MouseTester (desktop)  
  https://github.com/microe1/MouseTester

---

## Keyboard Testing

- **GearTest – Keyboard Test**  
  Browser-based keyboard test for verifying key registration and detecting ghosting.  
  https://www.geartest.online/keyboard-test/

- AquaKeyTest  
  https://www.aquatuning.com/aquakeytest

---

## Audio Input / Output Testing

- **GearTest – Audio Test**  
  Online audio test for microphone input and speaker output.  
  https://www.geartest.online/audio-test/

- Online Mic Test  
  https://www.onlinemictest.com/

---

## Scope

This list focuses on practical input device diagnostics  
(mouse, keyboard, and audio), rather than full hardware testing suites.

